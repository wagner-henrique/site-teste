...
export async function POST(req: Request) {
  try {
    ...
    const adminEmailSubject = `Novo Contato de ${name}`;
    ...
    await sendAdminEmail({
      to: process.env.NODEMAILER_TO_EMAIL as string,
    ...
    const userEmailSubject = `Confirmação de Contato - ${process.env.NODEMAILER_FROM_EMAIL}`; // Assunto para o usuário
    const userEmailHtml = `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
